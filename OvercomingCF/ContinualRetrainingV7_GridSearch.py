...
# ======================================================================
def objective(original_tasks_acc_threshold,retrained_model):#(args):#(ewc_lambda, lr_value_retrain): 
	...
	# 	retrained_model,_,_ = train_ewc(retrained_model, device, 1, accumilated_retraining_set.train_X, accumilated_retraining_set.train_Y, optimizer, epoch, ewc_lambdas, fisher_dict, optpar_dict)
	...
	for i in range(10):
		array_of_original_tasks_scores.append(original_tasks_acc_threshold - test(retrained_model, device, X_test[np.where(Y_test==i)], Y_test[np.where(Y_test==i)]))

	original_tasks_score   = np.max(array_of_original_tasks_scores)

	# Get the score of PoL tasks on the retrained model
	accumilted_tasks_score = test(retrained_model, device, accumilated_retraining_set.test_X, accumilated_retraining_set.test_Y)
	...
	try:
		...
		retraining_score = 100000 #just a big number.
		print("WARNING:  Rretraing score got divided by zero")
	...
	if Retraining_flag == True :
		...
		for current_ID in range(n_retraining_sets):
			...
			for temp_model in temp_models_array:
				...
				temp_retraining_scores_array.append(objective(original_tasks_acc_threshold,temp_model))
			...
			# == Method 4 of scoring ======================================================
			# Out of the retraining scores choose the minmum as this corresponds to the highest performance on the new data and attaching to learning without forgetting.
			max_original_data_acc = min(temp_retraining_scores_array)
			index_of_best_scoring_model = np.where(temp_retraining_scores_array == max_original_data_acc)
			print(index_of_best_scoring_model)
			index_of_best_scoring_model = index_of_best_scoring_model[0][0]
			print(index_of_best_scoring_model)
			# index_of_best_scoring_model = temp_indicies[max_index[0]][0]
...
file_name1     = "results_combined_26-1.csv"
file_name2     = "results_combined_26-2.csv"
file_name3     = "results_combined_26-3.csv"
